...
const debug = require("debug")("backend:module:jenis-kelamin");

const DB = {
  JenisKelamin: require("../models/jenis_kelamin.model"),
};
...
async function read(query, curUser) {
  try {
    ...
    if (query?.showAll) {
      found_data = await DB.JenisKelamin.find(filter)
        .sort(sort)
    ...
    if (!isEmptyArray(found_data)) {
      const count = await DB.JenisKelamin.countDocuments(filter).lean();

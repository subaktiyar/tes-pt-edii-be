...
const debug = require("debug")("backend:module:tingkat_pendidikan");

const DB = {
  TingkatPendidikan: require("../models/tingkat_pendidikan.model"),
};
...
async function read(query, curUser) {
  try {
    ...
    if (query?.showAll) {
      found_data = await DB.TingkatPendidikan.find(filter)
        .sort(sort)
    ...
    if (!isEmptyArray(found_data)) {
      const count = await DB.TingkatPendidikan.countDocuments(filter).lean();

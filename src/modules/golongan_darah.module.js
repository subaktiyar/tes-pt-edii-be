...
const debug = require("debug")("backend:module:golongan-darah");

const DB = {
  GolonganDarah: require("../models/golongan_darah.model"),
};
...
async function read(query, curUser) {
  try {
    ...
    if (query?.showAll) {
      found_data = await DB.GolonganDarah.find(filter)
        .sort(sort)
    ...
    if (!isEmptyArray(found_data)) {
      const count = await DB.GolonganDarah.countDocuments(filter).lean();

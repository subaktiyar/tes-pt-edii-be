...
const debug = require("debug")("backend:module:posisi");

const DB = {
  Posisi: require("../models/posisi.model"),
};
...
async function read(query, curUser) {
  try {
    ...
    if (query?.showAll) {
      found_data = await DB.Posisi.find(filter)
        .sort(sort)
    ...
    if (!isEmptyArray(found_data)) {
      const count = await DB.Posisi.countDocuments(filter).lean();
